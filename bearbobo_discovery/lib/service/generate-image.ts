export async function generateImage(prompt: string): Promise<{ error: string, url: string }> {
    const endpoint = process.env.VITE_KLING_END_POINT;
    const modelName = process.env.VITE_KLING_MODEL_NAME;
    ...
    const headers: any = {
        ...
        'X-Access-Key': process.env.VITE_ACCESS_KEY,
        'X-Secret-Key': process.env.VITE_SECRET_KEY,
    };
